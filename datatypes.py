...
converter.register_structure_hook(datetime, lambda d, _: datetime.strptime(d, "%Y-%m-%d").date())
...
class JobPosting:
    ...
    def from_tag(cls, job_posting: Tag):
        ...
        salary_description = [s.string for s in job_posting.find_all("p") if s.string and "hiring range" in s.string]
        if salary_description:
            ...
            salary_range_tuple = tuple([int(re.sub("\$|\,", "", s)) for s in salary_range]) or None

        return cls(
            role=job_posting.h1.string,
            date=datetime.strptime(article.find("span", class_="date").string, "%B %d, %Y").date(),
