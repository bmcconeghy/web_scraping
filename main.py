...
def plot_salary_ranges(df: pd.DataFrame, salary_to_benchmark_against: int):
    ...
    fig = px.bar(df.sort_values("Low End", ascending=False), y="role", x=["High End", "Low End"], barmode="group", labels={"variable": "Salary", "role": "Role", "value": "$"})
    fig = fig.update_layout(title=dict(text="Salaries at AbCellera (Job Postings)", font=dict(size=30)))
    fig.add_vline(x=salary_to_benchmark_against, line_width=3, line_dash="dash", line_color="green")
    fig.add_annotation(
        ...
        arrowhead=2,
        arrowsize=1,
        arrowwidth=2, text="Salary to compare")
    return fig


if __name__ == "__main__":    
    DEFAULT_SALARY = 50000
    user_salary = input(f"Please enter salary to benchmark against (in integer format, e.g. 50000 for $50,000)\n(Press ENTER to set benchmark salary to ${DEFAULT_SALARY:,})\n") or DEFAULT_SALARY
